...
def convert_info(
    ...
    # --- PATCH: Downgrade HF Schema from 'List' to 'Sequence' --- # openpi
    import json
    info_str = json.dumps(info).replace('"_type": "List"', '"_type": "Sequence"')
...
def convert_data(
    ...
    for (chunk_idx, file_idx), records in tqdm.tqdm(grouped.items(), desc="convert data files"):
        ...
        # --- PATCH: Downgrade HF Parquet Metadata ---
        metadata = table.schema.metadata
        if metadata and b"huggingface" in metadata:
            hf_meta_str = metadata[b"huggingface"].decode("utf-8")
            hf_meta_str = hf_meta_str.replace('"_type": "List"', '"_type": "Sequence"')
            metadata[b"huggingface"] = hf_meta_str.encode("utf-8")
